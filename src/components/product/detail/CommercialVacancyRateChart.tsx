...
const CommercialVacancyRateChart: React.FC = () => {
  ...
  const { data, error, isLoading } = useQuery({
    queryKey: ['vacancyRateData'],
    queryFn: fetchData
  });

  // 데이터를 필터링하고 변환하는 함수
  const transformAndFilterData = (): ChartData => {
    if (!data || !data.vacancyrate) return { labels: [], datasets: [] };

    // 사용자가 선택한 기간에 맞게 데이터를 필터링
    const filteredVacancyRates = data.vacancyrate.filter((item) => {
    ...
    const labels = Array.from(
      new Set(filteredVacancyRates.map((item) => `${item.year}.${item.quarter}Q`))
    );

    // 지역별로 데이터셋 생성
    const regions = Array.from(new Set(filteredVacancyRates.map((item) => item.region)));
    ...
    const datasets = regions.map((region) => {
      ...
      const regionData = filteredVacancyRates.filter((item) => item.region === region);
      return {
        label: region,
        data: labels.map((label) => {
          ...
          return quarterData ? quarterData.vacancyRate : null; // 데이터가 없을 경우 null 반환
        }),
    ...
    <>
      ...
      <div className="flex flex-col items-center justify-center h-full bg-gray-50 mb-[100px]">
        <div className="w-full max-w-4xl h-[400px]">
          <Bar ref={chartRef} data={filteredData} options={options} />
        </div>
      </div>
    </>
  );
