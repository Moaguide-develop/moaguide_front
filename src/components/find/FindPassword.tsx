...
import { LoadingSpinner } from '../../app/signup/(signup)/modal/LoadingSpinner';
    ...
    <div className="max-w-[330px] mx-auto min-h-[calc(100dvh-75.5px)] flex flex-col items-center justify-between sm:min-h-[100vh] sm:justify-center">
      {!showPasswordReset && (
        <>
          ...
          <div className="w-[90%] sm:w-full max-w-[340px]">
            {isComplete ? (
              <div
        ...
        <>
            ...
            <section>
              ...
              <div className="mt-10">
                ...
                ${password ? (passwordValid ? 'outline-success' : 'focus:outline-normal') : ''}`}
                  />
                  {!password && (
                    <p className="w-full mr-auto text-[#6E6F73] text-xs mt-2">
                      영문, 숫자, 특수문자 포함 8-20자로 입력해주세요.
                    </p>
                  )}
                  {password && passwordValid === false && (
                    <p className="w-full mr-auto  text-error text-xs mt-2">
                      비밀번호 양식에 맞지 않습니다.
                    </p>
                  )}
                  {passwordValid === true && (
                    <p className="w-full mr-auto text-success text-xs mt-2">
                      사용 가능한 비밀번호입니다.
                    </p>
                  )}
                </div>
          ...
          <motion.div
            ...
            {passwordValid && passwordMatch ? (
              <div
                className="flex items-center justify-center px-5 py-3 w-full rounded-[12px] font-bold text-lg bg-gradient2 text-heading4 text-white cursor-pointer mt-0 sm:mt-[40px] mb-[20px] sm:mb-0"
